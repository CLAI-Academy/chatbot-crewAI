        ...
        {isAi ? (
          <div className="prose prose-invert prose-sm md:prose-base max-w-none overflow-x-auto">
            <ReactMarkdown components={{
              ...
              ul: ({ node, ...props }) => (
                <ul className="list-disc pl-5 space-y-1 my-4 break-words" {...props} />
              ),
              ol: ({ node, ...props }) => (
                <ol className="list-decimal pl-5 space-y-1 my-4 break-words" {...props} />
              ),
              li: ({ node, ...props }) => (
                <li className="ml-4 break-words" {...props} />
              ),
              h1: ({ node, ...props }) => (
                <h1 className="text-2xl font-bold mt-6 mb-4 break-words" {...props} />
              ),
              h2: ({ node, ...props }) => (
                <h2 className="text-xl font-bold mt-5 mb-3 break-words" {...props} />
              ),
              h3: ({ node, ...props }) => (
                <h3 className="text-lg font-bold mt-4 mb-2 break-words" {...props} />
              ),
              hr: ({ node, ...props }) => (
