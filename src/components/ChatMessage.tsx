...
// Helper function to detect investment report format
const isInvestmentReport = (content: string): boolean => {
  return (
    content.includes("# Estrategia de Inversión Optimizada") ||
    content.includes("# Informe de Estrategia de Inversión") ||
    content.includes("Tabla de Escenarios de Inversión") ||
    (content.includes("Estrategia") &&
      content.includes("Inversión") &&
      content.includes("Escenario") &&
      content.includes("Riesgo") &&
      content.includes("Monto"))
  );
};
...
const ChatMessage: React.FC<{ message: MessageType }> = ({ message }) => {
  ...
  // Check if the message contains investment report data
  const containsInvestmentReport = isAi && isInvestmentReport(message.content);
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "rounded-xl p-4 max-w-[90%] overflow-hidden",
          isAi
      ...
      >
        {isAi ? (
          containsInvestmentReport ? (
            <InvestmentReport content={message.content} />
          ) : (
